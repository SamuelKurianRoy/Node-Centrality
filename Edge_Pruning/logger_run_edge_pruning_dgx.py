...
def setup_logging(log_dir="logs"):
    ...
    Creates timestamped log files with detailed formatting.
    """
    ...
    log_filename = os.path.join(log_dir, f"edge_pruning_{DATASET_NAME}_{timestamp}.log")
    
    # Create logger
    logger = logging.getLogger('EdgePruning')
...
class TeeLogger:
    """Helper class to redirect print statements to logger."""
...
def worker_eval_random_edges(chunk_range, baseline_state_cpu, prune_ratio, input_size, 
                             ...
                             top_k_local, logger=None, log_every=LOG_EVERY):
    """
    ...
    for idx in chunk_range:
        ...
        if (idx + 1) % log_every == 0:
            msg = f"[GPU {device_index}] Completed {idx + 1}/{num_trials} trials"
            if logger:
                logger.debug(msg)
            else:
                print(msg)
...
def spectral_edge_prune_and_metrics(prune_ratio, baseline_model, baseline_acc, 
                                   ...
                                   num_classes, dataset_name, finetune_epochs, logger=None):
    ...
    if logger:
        logger.info(f"Accuracy after spectral pruning (before finetune): {acc_pruned:.2f}%")
...
def main():
    ...
    logger.info("="*70)
    logger.info("DGX EDGE PRUNING PIPELINE - Spectral vs Random")
    logger.info("="*70)
    logger.info(f"Start time: {datetime.now().isoformat()}")
    logger.info(f"Dataset: {DATASET_NAME}")
    logger.info(f"Prune ratios: {PRUNE_RATIOS}")
    logger.info(f"Random trials per ratio: {NUM_RANDOM_TRIALS}")
    logger.info(f"Top-K candidates to finetune: {TOP_K}")
    logger.info(f"CPUs: {psutil.cpu_count(logical=True)}, RAM: {psutil.virtual_memory().total/1e9:.2f} GB")
    
    # Detect GPUs
    devices = get_available_gpus()
    if not devices:
        logger.warning("No available GPUs detected. Running on CPU (slow).")
        devices = []
    else:
        logger.info(f"Selected GPUs: {devices}")
    ...
    logger.info(f"Temp directory: {tmp_root}")
    
    # Load dataset and train baseline
    logger.info("")
    logger.info("="*70)
    logger.info("LOADING DATASET AND TRAINING BASELINE")
    logger.info("="*70)
    ...
    logger.info(f"Training baseline model on {baseline_dev} for {BASELINE_EPOCHS} epochs...")
    ...
    logger.info(f"Baseline training completed in {baseline_elapsed/60:.2f} minutes")
    logger.info(f"Baseline accuracy: {baseline_acc:.2f}%")
    logger.info(f"Baseline params: {params_base:,} (nonzero: {nonzero_base:,})")
    logger.info(f"Baseline FLOPs: {flops_base:,}")
    ...
    for prune_ratio in PRUNE_RATIOS:
        logger.info("")
        logger.info("="*70)
        logger.info(f"PROCESSING PRUNE RATIO: {prune_ratio:.2f} ({int(prune_ratio*100)}% edges removed)")
        logger.info("="*70)
        
        # Partition random trials across workers
        chunks = chunk_ranges(NUM_RANDOM_TRIALS, num_workers)
        logger.info(f"Total trials: {NUM_RANDOM_TRIALS}, Workers: {num_workers}")
        ...
        logger.info("")
        logger.info("--- Phase 1: Fast Random Edge Pruning Evaluation ---")
        fast_start = time.time()
        all_local_candidates = []
        
        if num_workers == 1 and not devices:
            # CPU fallback
            logger.info("Running on CPU (single worker)")
            local = worker_eval_random_edges(chunks[0], baseline_state_cpu, prune_ratio, 
                                           input_size, num_classes, DATASET_NAME, None, 
                                           worker_tempdirs[0], TOP_K, logger)
            all_local_candidates.extend(local)
        else:
            logger.info(f"Launching {num_workers} parallel workers across GPUs: {devices}")
            with ProcessPoolExecutor(max_workers=num_workers) as exe:
                futures = []
                for i, chunk in enumerate(chunks):
                    dev = devices[i % len(devices)]
                    logger.debug(f"Worker {i}: GPU {dev}, trials {chunk.start}-{chunk.stop-1}")
                    futures.append(exe.submit(worker_eval_random_edges, chunk, 
                                            ...
                                            worker_tempdirs[i], TOP_K, None))
                
                for i, fut in enumerate(as_completed(futures)):
                    try:
                        res = fut.result()
                        all_local_candidates.extend(res)
                        logger.info(f"Worker {i} completed, returned {len(res)} candidates")
                    except Exception as e:
                        logger.error(f"Worker {i} error during eval: {e}", exc_info=True)
        
        fast_elapsed = time.time() - fast_start
        logger.info(f"Fast eval completed in {fast_elapsed/60.0:.2f} minutes")
        logger.info(f"Collected {len(all_local_candidates)} local candidates")
        
        # Select global top-K
        all_local_candidates.sort(key=lambda x: x["acc"], reverse=True)
        top_candidates = all_local_candidates[:TOP_K]
        logger.info(f"Selected global top {len(top_candidates)} candidates")
        if top_candidates:
            logger.info(f"Best candidate accuracy: {top_candidates[0]['acc']:.2f}%")
            logger.debug(f"Top-5 accuracies: {[f\"{c['acc']:.2f}%\" for c in top_candidates[:5]]}")
        ...
        logger.info("")
        logger.info("--- Phase 2: Finetuning Top Candidates ---")
        ...
        if top_candidates:
            if not devices:
                # CPU fallback
                logger.info("Finetuning on CPU sequentially")
                for idx, cand in enumerate(top_candidates):
                    logger.info(f"Finetuning candidate {idx+1}/{len(top_candidates)}")
                    res = worker_finetune_candidate(cand, baseline_state_cpu, prune_ratio,
                                                   input_size, num_classes, DATASET_NAME,
                                                   None, FINETUNE_EPOCHS, logger)
                    finetune_results.append(res)
            else:
                logger.info(f"Finetuning {len(top_candidates)} candidates in parallel")
                with ProcessPoolExecutor(max_workers=min(len(top_candidates), len(devices))) as exe:
                    futures = []
                    for i, cand in enumerate(top_candidates):
                        dev = devices[i % len(devices)]
                        logger.debug(f"Finetuning candidate {i+1} on GPU {dev}")
                        futures.append(exe.submit(worker_finetune_candidate, cand, 
                                                baseline_state_cpu, prune_ratio, input_size,
                                                num_classes, DATASET_NAME, dev, FINETUNE_EPOCHS, None))
                    
                    for i, fut in enumerate(as_completed(futures)):
                        try:
                            res = fut.result()
                            finetune_results.append(res)
                            logger.info(f"Finetune {i+1}/{len(top_candidates)} completed: "
                                      f"Acc={res['Accuracy_After_Finetuning']:.2f}%")
                        except Exception as e:
                            logger.error(f"Finetune worker error: {e}", exc_info=True)
        
        finetune_elapsed = time.time() - finetune_start
        logger.info(f"Finetuning completed in {finetune_elapsed/60.0:.2f} minutes")
        logger.info(f"Collected {len(finetune_results)} finetune records")
        
        if finetune_results:
            best_ft = max(finetune_results, key=lambda x: x['Accuracy_After_Finetuning'])
            logger.info(f"Best finetuned accuracy: {best_ft['Accuracy_After_Finetuning']:.2f}%")
        
        aggregated_random_finetune.extend(finetune_results)
        
        # PHASE 3: SPECTRAL EDGE PRUNING
        logger.info("")
        logger.info("--- Phase 3: Spectral Edge Pruning (Fiedler Method) ---")
        try:
            ...
            spec_res = spectral_edge_prune_and_metrics(prune_ratio, baseline_model, 
                                                      ...
                                                      test_loader_main, baseline_dev, 
                                                      input_size, num_classes, DATASET_NAME,
                                                      FINETUNE_EPOCHS, logger)
            spectral_elapsed = time.time() - spectral_start
            aggregated_spectral.append(spec_res)
            logger.info(f"Spectral edge pruning completed in {spectral_elapsed/60:.2f} minutes")
            logger.info(f"Spectral accuracy (before finetune): {spec_res['Accuracy_After_Pruning']:.2f}%")
            logger.info(f"Spectral accuracy (after finetune): {spec_res['Accuracy_After_Finetuning']:.2f}%")
            logger.info(f"Spectral sparsity: {spec_res['Sparsity']*100:.2f}%")
        except Exception as e:
            logger.error(f"Spectral pruning failed: {e}", exc_info=True)
        ...
        logger.info(f"Saved CSVs: {out_csv_eval}, {out_csv_ft}, {out_csv_spec}")
        ...
        for d in worker_tempdirs:
            try:
                ...
                logger.debug(f"Could not remove temp dir {d}: {e}")
    ...
    logger.info(f"All results saved to: {final_excel}")
    
    # Cleanup temp directory
    try:
        shutil.rmtree(tmp_root)
        logger.info(f"Cleaned up temp directory: {tmp_root}")
    except Exception as e:

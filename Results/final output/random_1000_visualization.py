...
for idx, ratio in enumerate([0.2, 0.3, 0.4]):
    ...
    if ratio == 0.2:
        data = random_02
        title = f'Random Pruning - Prune Ratio 0.2 (n=1000)\nBar Chart of All Experiments'
    elif ratio == 0.3:
        data = random_03
        title = f'Random Pruning - Prune Ratio 0.3 (n=1000)\nBar Chart of All Experiments'
    else:
        data = random_04
        title = f'Random Pruning - Prune Ratio 0.4 (n=1000)\nBar Chart of All Experiments'
...
for idx, ratio in enumerate([0.2, 0.3, 0.4]):
    ...
    ax2 = ax.twinx()
    ax2.plot(x_range, kde(x_range), 'k-', linewidth=2, label='KDE')
...
ax.set_ylabel('Accuracy (%)', fontsize=11, fontweight='bold')
ax.set_title('Box Plot Comparison - All Ratios\n(1000 Random Experiments Each)', 
...
ax.hist(all_training_times, bins=30, alpha=0.7, color='purple', edgecolor='black', linewidth=0.7)
ax.axvline(all_training_times.mean(), color='red', linestyle='--', linewidth=2.5,
          label=f'Mean: {all_training_times.mean():.2f}s')
ax.set_xlabel('Training Time (seconds)', fontsize=11, fontweight='bold')
ax.set_ylabel('Frequency', fontsize=11, fontweight='bold')
ax.set_title('Training Time Distribution\n(All 3000 Random Experiments)', 
            fontsize=12, fontweight='bold')
...
for ratio, color, label in [(0.2, '#1f77b4', 'Ratio 0.2'),
                             (0.3, '#ff7f0e', 'Ratio 0.3'),
                             (0.4, '#2ca02c', 'Ratio 0.4')]:
    if ratio == 0.2:
    ...
    ax.plot(sorted_data, cumulative, marker='.', linestyle='none', color=color, 
           alpha=0.6, markersize=3, label=label)
...
plt.tight_layout()
plt.savefig('Random_1000_Experiments_Visualization.png', dpi=300, bbox_inches='tight',
           facecolor='white', edgecolor='none')
print("\n✓ Visualization saved as 'Random_1000_Experiments_Visualization.png'")
plt.show()
